from .models import Category,ProductCategoryMapping,Item,OrderItem,Order,Address,Payment,Refund,Coupon,Review,UserProfile,WishlistItem,ItemReviewMapping
import random
import string
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            if form.is_valid():
                ...
                if payment_option == 'S':
                    return redirect('mainapp:payment', payment_option='stripe')
                elif payment_option == 'P':
                    return redirect('mainapp:payment', payment_option='paypal')
                else:
                    messages.warning(
...
def ProductsView(request,id):
    pro = ProductCategoryMapping.objects.filter(category = id).values()
    # temp = Product.objects.filter()
    pros = []
    for it in pro:
        pro = Item.objects.get(id = it['product_id'])
...
def remove_from_wishlist(request,slug):
    item = get_object_or_404(Item,slug = slug)
    WishlistItem.objects.filter(item_id = item.id).delete()
    return redirect("mainapp:wishlist-summary")

def WishlistSummaryView(request):
    items = WishlistItem.objects.filter(user = request.user).values()

    pros = []
    for item in items:
        pro = Item.objects.get(id = item['item_id'])
        temp = {
            'title' : pro.title,
            'image' : pro.image,
            'slug' : pro.slug,
    ...
    return render(request,'wishlist.html',{'items':pros})
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                ...
                return redirect("mainapp:request-refund")
